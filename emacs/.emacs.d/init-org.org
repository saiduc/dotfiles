...
Get flymake working with C++, which is a little annoying
#+begin_src emacs-lisp
...
;; make flymake only check after each save
...
(add-hook 'c++-mode-hook 'flymake-mode)
(add-hook 'c++-mode-hook (lambda () (diminish 'flymake-mode)))
#+end_src

Get completion of Header files
#+begin_src emacs-lisp
