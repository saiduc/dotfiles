...
(use-package lsp-latex
  ...
  (setq lsp-latex-lint-on-change nil)
  ...
  (defun my/open-pdf ()
    ...
    (shell-command (concat "open " (replace-regexp-in-string ".tex" ".pdf" buffer-file-name)))
    )

  :config
  (define-key latex-mode-map (kbd "C-c C-c") 'lsp-latex-build)
  (define-key latex-mode-map (kbd "C-c C-v") 'my/open-pdf))
#+end_src
...
Get completion of Header files. It might be necessary to change the path to header files depending on the system you are on.
#+begin_src emacs-lisp
