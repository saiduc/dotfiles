...
(use-package org
  ...
  ;; function so sync google calendars
  (defun my/sync-google-calendars ()
    (interactive)
    (call-process "~/dotfiles/scripts/sync_personal_gcal" nil 0 nil)
    (call-process "~/dotfiles/scripts/sync_sasia_gcal" nil 0 nil)
    (message "Synced Google calendars!"))
...
(use-package elfeed
  ...
  (defvar my/elfeed-tags '("All"
			   ...
			   "Subreddits"
			   "Data-Science")
    "List of Elfeed tags for quick selection.")

  (defun my/elfeed-choose-category ()
    "Prompt to select an Elfeed tag using Vertico and filter Elfeed to that tag."
    (interactive)
    (require 'consult)
    (let ((tag (consult--read my/elfeed-tags
                              :prompt "Choose Elfeed tag: ")))
      (if (string= tag "All")
          (elfeed-search-set-filter "@6-months-ago +unread")
	(if (string= tag "Starred")
	  (elfeed-search-set-filter "+star")
	(elfeed-search-set-filter (concat "@6-months-ago +unread +" tag))))))


  (defun my/elfeed-toggle-star ()
  "Toggle the 'star' tag on selected entries in Elfeed."
  (interactive)
  (save-excursion
    (let ((entries (elfeed-search-selected)))
      (message "Selected %d entries" (length entries))
      (dolist (entry entries)
        (if (member 'star (elfeed-entry-tags entry))
            (elfeed-untag entry 'star)
          (elfeed-tag entry 'star))
        (elfeed-search-update-entry entry))
      (message "Toggled star tag for %d entries" (length entries)))))
  
  (define-key elfeed-show-mode-map (kbd "C-x C-t") 'my/elfeed-choose-category)
  (define-key elfeed-search-mode-map (kbd "C-x C-t") 'my/elfeed-choose-category)
  (define-key elfeed-search-mode-map (kbd "C-x C-s") 'my/elfeed-toggle-star)
  (define-key elfeed-search-mode-map (kbd "C-x C-s") 'my/elfeed-toggle-star))
...
(use-package elfeed-goodies
  ...
  :after elfeed
  :init
  (setq elfeed-goodies/entry-pane-size 0.66)
  :config
...
(use-package elfeed-protocol
  ...
  (setq elfeed-protocol-feeds '(("fever+http://saipandian@192.168.1.249:1008"
				 :api-url "http://192.168.1.249:1008/api/fever.php"
				 :password "ChangeMe")))
...
Sometimes we need this to get the path correctly
#+begin_src emacs-lisp
...
(use-package lsp-mode
  ...
  ;; might need to copy this line before and outside of the
  ;; use-package macro, as it needs to be enabled when the
  ;; package is installed and compiled
  ;; also need to make sure the plist line works in the
  ;; early-init or set in the terminal and pulled in with
...
Speeding up lsp mode with lsp-booster. To use this, you need to download the
binary from [[https://github.com/blahgeek/emacs-lsp-booster][the repo]] and place it in =~/.local/bin=. This will also only work if
the plist stuff is working for lsp-mode (which it should if this config still
works).
#+begin_src emacs-lisp
...
** Jupyter
I use Emacs Jupyter for jupyter notebooks when I can, as EIN can be a little buggy
#+begin_src emacs-lisp
(use-package jupyter
  :straight t
  :defer t
...
(use-package ox-ipynb
  ...
  :defer t
  :init
  (defun my/load-ox-ipynb ()
    "Loads ox ipynb"
    (interactive)
    (lambda () (require 'ox-ipynb))))
#+end_src
...
Install using: brew install texlab or conda install conda-forge::texlab
#+begin_src emacs-lisp
(use-package lsp-latex
  ...
  (setq lsp-latex-texlab-executable "/home/saipandian/miniconda3/bin/texlab")
