...
Then do C-u C-c-v-k to delete the outputs.
...
#+RESULTS:
| ~/.emacs.d/lisp | /Users/saipandian/.emacs.d/elpa/use-package-20191126.2034 | /Users/saipandian/.emacs.d/elpa/bind-key-20191110.416 | /usr/local/share/emacs/site-lisp | /usr/local/share/emacs/site-lisp/cmake | /usr/local/share/emacs/site-lisp/git | /usr/local/share/emacs/site-lisp/libidn | /usr/local/Cellar/emacs-plus/26.3/share/emacs/26.3/lisp | /usr/local/Cellar/emacs-plus/26.3/share/emacs/26.3/lisp/vc | /usr/local/Cellar/emacs-plus/26.3/share/emacs/26.3/lisp/url | /usr/local/Cellar/emacs-plus/26.3/share/emacs/26.3/lisp/textmodes | /usr/local/Cellar/emacs-plus/26.3/share/emacs/26.3/lisp/progmodes | /usr/local/Cellar/emacs-plus/26.3/share/emacs/26.3/lisp/play | /usr/local/Cellar/emacs-plus/26.3/share/emacs/26.3/lisp/org | /usr/local/Cellar/emacs-plus/26.3/share/emacs/26.3/lisp/nxml | /usr/local/Cellar/emacs-plus/26.3/share/emacs/26.3/lisp/net | /usr/local/Cellar/emacs-plus/26.3/share/emacs/26.3/lisp/mh-e | /usr/local/Cellar/emacs-plus/26.3/share/emacs/26.3/lisp/mail | /usr/local/Cellar/emacs-plus/26.3/share/emacs/26.3/lisp/leim | /usr/local/Cellar/emacs-plus/26.3/share/emacs/26.3/lisp/language | /usr/local/Cellar/emacs-plus/26.3/share/emacs/26.3/lisp/international | /usr/local/Cellar/emacs-plus/26.3/share/emacs/26.3/lisp/image | /usr/local/Cellar/emacs-plus/26.3/share/emacs/26.3/lisp/gnus | /usr/local/Cellar/emacs-plus/26.3/share/emacs/26.3/lisp/eshell | /usr/local/Cellar/emacs-plus/26.3/share/emacs/26.3/lisp/erc | /usr/local/Cellar/emacs-plus/26.3/share/emacs/26.3/lisp/emulation | /usr/local/Cellar/emacs-plus/26.3/share/emacs/26.3/lisp/emacs-lisp | /usr/local/Cellar/emacs-plus/26.3/share/emacs/26.3/lisp/cedet | /usr/local/Cellar/emacs-plus/26.3/share/emacs/26.3/lisp/calendar | /usr/local/Cellar/emacs-plus/26.3/share/emacs/26.3/lisp/calc | /usr/local/Cellar/emacs-plus/26.3/share/emacs/26.3/lisp/obsolete |
...
Keep this block in this file and don't move it to early-init.el because it is
easier to keep track of if you leave macOS
#+BEGIN_SRC emacs-lisp
...
* Mode Line
Spaceline is a pretty complete option
#+BEGIN_SRC emacs-lisp
...
Doom modeline is a more lightweight option
#+begin_src emacs-lisp
...
* Themes
Some options for themes are below:
#+begin_src emacs-lisp
...
This function disables previous theme before enabling new one. This prevents the
clashing between themes you often see if you just load a new theme. It then
refreshes spaceline and reapplies custom org faces.
#+begin_src emacs-lisp
...
need to have cmake installed for this, which can be done in mac with: brew
install cmake
#+begin_src emacs-lisp
(use-package vterm
  :ensure t
...
# I make ido work vertically and navigate with vim keys
# #+BEGIN_SRC emacs-lisp
...
# I want to use ido to complete pretty much every list so:
# #+BEGIN_SRC emacs-lisp
...
* Ivy & Counsel
I am considering using Ivy & Counsel
They are slightly heavier than just ido-mode and smex but there may be some cool
features only available with counsel. As of now, there aren't any
counsel-specific things I use, but perhaps in the future.
#+begin_src emacs-lisp
(use-package smex
  :ensure t
  :defer t)
...
(use-package counsel
  ...
  (setq counsel-fzf-cmd "ag --hidden --ignore .git -g '' | fzf -f \'%s\'")
  (counsel-mode 1))
...
* Python
Choice of Emacs-Jedi, Anaconda-mode, or Elpy I am using Elpy because it remains
fast and is feature rich. You will need to make your own elpy virtual
environment with conda, and point to it with the elpy-rpc-virtualenv-path variable
#+BEGIN_SRC emacs-lisp
(use-package elpy
  :ensure t
  :defer t
  :init
  (setq elpy-rpc-virtualenv-path "/Users/saipandian/miniconda3/envs/elpy")
  ...
  ;; should be in bind:
  (add-hook 'elpy-ve-mode-hook (lambda () (display-line-numbers-mode -1)))
  (add-hook 'elpy-mode-hook
	    (lambda ()
	      (define-key elpy-mode-map (kbd "M-]") 'elpy-goto-definition)
	      (define-key elpy-mode-map (kbd "M-[") 'pop-tag-mark)
	      (define-key elpy-mode-map (kbd "C-c C-f") 'projectile-find-file)))
...
Jedi, but neither offer as many features as Elpy. A language server might be
cool but the python one is slow for large files and you have to install it in
...
* Cython
Sometimes I use cython for the best of both C and Python
#+BEGIN_SRC emacs-lisp
...
* Jupyter
I use Emacs Jupyter for jupyter notebooks. I used to use ein, but it's a little
slow and buggy
#+BEGIN_SRC emacs-lisp
(use-package jupyter
  :ensure t
...
melpa. If you don't have it, download it from: [[https://github.com/jkitchin/ox-ipynb][ox-ipynb]] and put it in
.emacs.d/lisp/
#+BEGIN_SRC emacs-lisp
...
* C/C++
I use irony mode for C and C++
You will need to install libclang to get this to work properly

On macOS, this is done with:
brew --cc clang llvm
Then, you will need to run (irony-install-server), then navigate to .emacs.d/irony/bin and run:
install_name_tool -change @rpath/libclang.dylib /usr/local/opt/llvm/lib/libclang.dylib irony-server
Then you can run the (irony-install-server) command again with the options:
...
More details for installation at [[https://gist.github.com/soonhokong/7c2bf6e8b72dbc71c93b][GithubGist]]
...
Set up c++ to use the style I like, the linux kernel style:
#+begin_src emacs-lisp
...
* Org Mode
Org mode seems to require an extra package after version 9.2 so ensure that it
is loaded
#+BEGIN_SRC emacs-lisp
...
This gets org mode working with python and jupyter
#+BEGIN_SRC emacs-lisp
...
(custom-set-faces
  '(org-level-1
    ((t (:inherit outline-1 :height 1.2)))))
...
** In Emacs 27
*** Use package quickstart (just set it to true in init.el)
*** Use early-init for the gui things
*** Get rid of package-initialize
** If I ever feel like it
*** Set up spacemacs keybindings
**** [[https://gist.github.com/adham90/0d85d023f9d4f1dd28ef#file-spacemacs-keybindings][List of all bindings]]
* TESTGROUND
